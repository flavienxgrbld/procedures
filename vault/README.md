...
- **vault** - Service systÃ¨me avec dÃ©marrage automatique
...
- /etc/vault/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/vault/ - DonnÃ©es de l'application
- /var/log/vault/ - Logs de l'application
...
sudo chown -R True:True /var/lib/vault/
...
sudo tar -czf backup-vault-$(date +%Y%m%d).tar.gz /var/lib/vault/

# Restaurer une sauvegarde
sudo tar -xzf backup-vault-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade vault  # Debian/Ubuntu
sudo dnf upgrade vault  # RedHat/Fedora
...
sudo cp /etc/vault/config /etc/vault/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall vault  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract vault /etc/
`
