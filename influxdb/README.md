...
- **influxdb** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8086 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/influxdb/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/influxdb/ - DonnÃ©es de l'application
- /var/log/influxdb/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/influxdb/
`
...
sudo chown -R True:True /var/lib/influxdb/
...
sudo tar -czf backup-influxdb-$(date +%Y%m%d).tar.gz /var/lib/influxdb/

# Restaurer une sauvegarde
sudo tar -xzf backup-influxdb-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade influxdb  # Debian/Ubuntu
sudo dnf upgrade influxdb  # RedHat/Fedora
...
sudo cp /etc/influxdb/config /etc/influxdb/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall influxdb  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract influxdb /etc/
`
