...
Redis - Cache et base de donnÃ©es en mÃ©moire
...
| 6379 | tcp | Redis - Cache en mÃ©moire |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/redis/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/redis/ - DonnÃ©es de l'application
- /var/log/redis/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/redis/
`
...
sudo chown -R True:True /var/lib/redis/
...
sudo tar -czf backup-redis-$(date +%Y%m%d).tar.gz /var/lib/redis/

# Restaurer une sauvegarde
sudo tar -xzf backup-redis-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade redis  # Debian/Ubuntu
sudo dnf upgrade redis  # RedHat/Fedora
...
sudo cp /etc/redis/config /etc/redis/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall redis  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract redis /etc/
`
