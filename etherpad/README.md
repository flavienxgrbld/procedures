...
Crowdfunding - EtherPad alternative collaborative writing
...
chmod +x install_etherpad.sh

# 2. ExÃ©cutez le script d'installation
bash install_etherpad.sh
...
- **etherpad** - Service systÃ¨me avec dÃ©marrage automatique
...
- /etc/etherpad/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/etherpad/ - DonnÃ©es de l'application
- /var/log/etherpad/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/etherpad/
`
...
sudo chown -R True:True /var/lib/etherpad/
...
sudo rm -rf /opt/etherpad
sudo rm -rf /var/lib/etherpad
sudo rm -rf /etc/etherpad
...
sudo tar -czf backup-etherpad-$(date +%Y%m%d).tar.gz /var/lib/etherpad/

# Restaurer une sauvegarde
sudo tar -xzf backup-etherpad-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade etherpad  # Debian/Ubuntu
sudo dnf upgrade etherpad  # RedHat/Fedora
...
sudo cp /etc/etherpad/config /etc/etherpad/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall etherpad  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract etherpad /etc/
`
