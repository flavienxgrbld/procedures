# Installation de node_red

## Description

Node-RED - Programmation visuelle pour l'automation
...
- **node-red** - Service systÃ¨me avec dÃ©marrage automatique
...
| 1880 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/node_red/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/node_red/ - DonnÃ©es de l'application
- /var/log/node_red/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/node_red/
`
...
sudo chown -R True:True /var/lib/node_red/
...
sudo rm -rf /opt/node_red
sudo rm -rf /var/lib/node_red
sudo rm -rf /etc/node_red
...
sudo tar -czf backup-node_red-$(date +%Y%m%d).tar.gz /var/lib/node_red/

# Restaurer une sauvegarde
sudo tar -xzf backup-node_red-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade node_red  # Debian/Ubuntu
sudo dnf upgrade node_red  # RedHat/Fedora
...
sudo cp /etc/node_red/config /etc/node_red/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall node_red  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract node_red /etc/
`
