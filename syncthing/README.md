...
Syncthing - Synchronisation distribuÃ©e P2P
...
- **syncthing@$USER** - Service systÃ¨me avec dÃ©marrage automatique
...
| 21027 | udp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/syncthing/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/syncthing/ - DonnÃ©es de l'application
- /var/log/syncthing/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/syncthing/
`
...
sudo chown -R True:True /var/lib/syncthing/
...
sudo rm -rf /var/lib/syncthing
sudo rm -rf /etc/syncthing
...
sudo tar -czf backup-syncthing-$(date +%Y%m%d).tar.gz /var/lib/syncthing/

# Restaurer une sauvegarde
sudo tar -xzf backup-syncthing-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade syncthing  # Debian/Ubuntu
sudo dnf upgrade syncthing  # RedHat/Fedora
...
sudo cp /etc/syncthing/config /etc/syncthing/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall syncthing  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract syncthing /etc/
`
...
**TestÃ© sur** : apt (Debian/Ubuntu), dnf/yum (RedHat/Fedora), zypper (openSUSE), pacman (Arch)
