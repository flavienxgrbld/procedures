...
Typesense - Moteur de recherche rapide
...
chmod +x install_typesense.sh

# 2. ExÃ©cutez le script d'installation
bash install_typesense.sh
...
- **typesense** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8108 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/typesense/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/typesense/ - DonnÃ©es de l'application
- /var/log/typesense/ - Logs de l'application
...
sudo chown -R True:True /var/lib/typesense/
...
sudo rm -rf /opt/typesense
sudo rm -rf /var/lib/typesense
sudo rm -rf /etc/typesense
...
sudo tar -xzf backup-typesense-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade typesense  # Debian/Ubuntu
sudo dnf upgrade typesense  # RedHat/Fedora
...
sudo cp /etc/typesense/config /etc/typesense/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall typesense  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract typesense /etc/
`
