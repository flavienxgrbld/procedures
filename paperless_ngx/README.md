# Installation de paperless_ngx

## Description

Paperless-NGX - Gestion numÃ©rique de documents
...
- **paperless** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8000 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/paperless_ngx/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/paperless_ngx/ - DonnÃ©es de l'application
- /var/log/paperless_ngx/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/paperless_ngx/
`
...
sudo chown -R True:True /var/lib/paperless_ngx/
...
tail -f /var/log/paperless_ngx/*.log
`
...
sudo rm -rf /opt/paperless_ngx
sudo rm -rf /var/lib/paperless_ngx
sudo rm -rf /etc/paperless_ngx
...
- Forums de support
- Discord/Slack
- Stack Overflow (tag: paperless_ngx)
...
sudo tar -czf backup-paperless_ngx-$(date +%Y%m%d).tar.gz /var/lib/paperless_ngx/

# Restaurer une sauvegarde
sudo tar -xzf backup-paperless_ngx-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade paperless_ngx  # Debian/Ubuntu
sudo dnf upgrade paperless_ngx  # RedHat/Fedora
...
sudo cp /etc/paperless_ngx/config /etc/paperless_ngx/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall paperless_ngx  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract paperless_ngx /etc/
`
