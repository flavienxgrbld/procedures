...
systemctl is-enabled $($services[0] -orr 'service-name')
```
...
journalctl -u $($services[0] ?? 'service-name') -f

# Voir les derniers logs
journalctl -u $($services[0] ?? 'service-name') -n 50

# Voir tous les logs du service
journalctl -u $($services[0] ?? 'service-name')
```
...
sudo journalctl -u $($services[0] ?? 'service-name') -n 50

# Vérifier la syntaxe de configuration
sudo $($services[0] ?? 'service-name') --version

# Redémarrer le service
sudo systemctl restart $($services[0] ?? 'service-name')
...
sudo ss -tlnp | grep :$($ports[0].port ?? '8080')

# Ou
sudo lsof -i :$($ports[0].port ?? '8080')
...
sudo usermod -aG $($services[0] ?? 'service-name') `$USER

# Appliquer les permissions
sudo chown -R $($services[0] ?? 'service-name'):$($services[0] ?? 'service-name') /var/lib/$($folderName.ToLower())/
...
sudo ufw allow $($ports[0].port ?? '8080')

# Rechec de la connectivité
curl -v http://localhost:$($ports[0].port ?? '8080')
```
...
sudo systemctl stop $($services[0] ?? 'service-name')

# 2. Désactiver le service
sudo systemctl disable $($services[0] ?? 'service-name')
...
sudo rm /etc/systemd/system/$($services[0] ?? 'service-name').service
