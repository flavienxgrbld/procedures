...
SyncThing - Synchronisation P2P dÃ©centralisÃ©e
...
- **syncthing@root** - Service systÃ¨me avec dÃ©marrage automatique
...
- /var/lib/backblaze_sync/ - DonnÃ©es de l'application
...
sudo chown -R True:True /var/lib/backblaze_sync/
...
- Forums de support
- Discord/Slack
- Stack Overflow (tag: backblaze_sync)
...
sudo tar -xzf backup-backblaze_sync-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade backblaze_sync  # Debian/Ubuntu
sudo dnf upgrade backblaze_sync  # RedHat/Fedora
...
sudo apt install --reinstall backblaze_sync  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract backblaze_sync /etc/
`
