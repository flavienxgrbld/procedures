# Installation de mattermost_enhanced

## Description

Mattermost - Plateforme de collaboration d'entreprise
...
chmod +x install_mattermost_enhanced.sh

# 2. ExÃ©cutez le script d'installation
bash install_mattermost_enhanced.sh
...
- **mattermost** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8065 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/mattermost_enhanced/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/mattermost_enhanced/ - DonnÃ©es de l'application
- /var/log/mattermost_enhanced/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/mattermost_enhanced/
`
...
sudo chown -R True:True /var/lib/mattermost_enhanced/
...
tail -f /var/log/mattermost_enhanced/*.log
`
...
sudo rm -rf /opt/mattermost_enhanced
sudo rm -rf /var/lib/mattermost_enhanced
sudo rm -rf /etc/mattermost_enhanced
...
- Forums de support
- Discord/Slack
- Stack Overflow (tag: mattermost_enhanced)
...
sudo tar -czf backup-mattermost_enhanced-$(date +%Y%m%d).tar.gz /var/lib/mattermost_enhanced/

# Restaurer une sauvegarde
sudo tar -xzf backup-mattermost_enhanced-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade mattermost_enhanced  # Debian/Ubuntu
sudo dnf upgrade mattermost_enhanced  # RedHat/Fedora
...
sudo cp /etc/mattermost_enhanced/config /etc/mattermost_enhanced/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall mattermost_enhanced  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract mattermost_enhanced /etc/
`
