...
Odoo - ERP/CRM open source complet
...
- **odoo** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8072 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/odoo/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/odoo/ - DonnÃ©es de l'application
- /var/log/odoo/ - Logs de l'application
...
sudo chown -R True:True /var/lib/odoo/
...
sudo tar -czf backup-odoo-$(date +%Y%m%d).tar.gz /var/lib/odoo/

# Restaurer une sauvegarde
sudo tar -xzf backup-odoo-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade odoo  # Debian/Ubuntu
sudo dnf upgrade odoo  # RedHat/Fedora
...
sudo cp /etc/odoo/config /etc/odoo/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall odoo  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract odoo /etc/
`
