...
Jellyfin - Serveur multimÃ©dia open source
...
- **jellyfin** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8920 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/jellyfin/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/jellyfin/ - DonnÃ©es de l'application
- /var/log/jellyfin/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/jellyfin/
`
...
sudo chown -R True:True /var/lib/jellyfin/
...
sudo tar -czf backup-jellyfin-$(date +%Y%m%d).tar.gz /var/lib/jellyfin/

# Restaurer une sauvegarde
sudo tar -xzf backup-jellyfin-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade jellyfin  # Debian/Ubuntu
sudo dnf upgrade jellyfin  # RedHat/Fedora
...
sudo cp /etc/jellyfin/config /etc/jellyfin/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall jellyfin  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract jellyfin /etc/
`
