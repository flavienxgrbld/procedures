...
- **sonarqube** - Service systÃ¨me avec dÃ©marrage automatique
...
- /etc/sonarqube/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/sonarqube/ - DonnÃ©es de l'application
- /var/log/sonarqube/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/sonarqube/
`
...
sudo chown -R True:True /var/lib/sonarqube/
...
sudo tar -czf backup-sonarqube-$(date +%Y%m%d).tar.gz /var/lib/sonarqube/

# Restaurer une sauvegarde
sudo tar -xzf backup-sonarqube-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade sonarqube  # Debian/Ubuntu
sudo dnf upgrade sonarqube  # RedHat/Fedora
...
sudo cp /etc/sonarqube/config /etc/sonarqube/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall sonarqube  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract sonarqube /etc/
`
