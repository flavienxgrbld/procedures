# Installation de requestbin

## Description

Request Bin - DÃ©bogage de webhooks HTTP
...
chmod +x install_requestbin.sh

# 2. ExÃ©cutez le script d'installation
bash install_requestbin.sh
...
- **redis-server** - Service systÃ¨me avec dÃ©marrage automatique
- **requestbin** - Service systÃ¨me avec dÃ©marrage automatique
...
- /etc/requestbin/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/requestbin/ - DonnÃ©es de l'application
- /var/log/requestbin/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/requestbin/
`
...
sudo chown -R True:True /var/lib/requestbin/
...
sudo rm -rf /opt/requestbin
sudo rm -rf /var/lib/requestbin
sudo rm -rf /etc/requestbin
...
sudo tar -czf backup-requestbin-$(date +%Y%m%d).tar.gz /var/lib/requestbin/

# Restaurer une sauvegarde
sudo tar -xzf backup-requestbin-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade requestbin  # Debian/Ubuntu
sudo dnf upgrade requestbin  # RedHat/Fedora
...
sudo cp /etc/requestbin/config /etc/requestbin/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall requestbin  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract requestbin /etc/
`
