# Installation de landscape

## Description

Ubuntu Landscape - Gestion de machines Ubuntu
...
chmod +x install_landscape.sh

# 2. ExÃ©cutez le script d'installation
bash install_landscape.sh
...
- **landscape-server** - Service systÃ¨me avec dÃ©marrage automatique
...
| 443 | tcp | HTTPS - AccÃ¨s web sÃ©curisÃ© |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/landscape/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/landscape/ - DonnÃ©es de l'application
- /var/log/landscape/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/landscape/
`
...
sudo chown -R True:True /var/lib/landscape/
...
sudo tar -czf backup-landscape-$(date +%Y%m%d).tar.gz /var/lib/landscape/

# Restaurer une sauvegarde
sudo tar -xzf backup-landscape-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade landscape  # Debian/Ubuntu
sudo dnf upgrade landscape  # RedHat/Fedora
...
sudo cp /etc/landscape/config /etc/landscape/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall landscape  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract landscape /etc/
`
