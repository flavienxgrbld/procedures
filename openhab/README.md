# Installation de openhab

## Description

OpenHAB - Domotique universelle
...
chmod +x install_openhab.sh

# 2. ExÃ©cutez le script d'installation
bash install_openhab.sh
...
- **openhab** - Service systÃ¨me avec dÃ©marrage automatique
...
| 8443 | tcp | Application |
## Configuration

### Configuration de Base

Les fichiers de configuration se trouvent gÃ©nÃ©ralement dans :
- /etc/openhab/ - Configuration de l'application
- /etc/systemd/system/ - Configuration des services
- /var/lib/openhab/ - DonnÃ©es de l'application
- /var/log/openhab/ - Logs de l'application
...
sudo chown -R $(whoami):$(whoami) /var/lib/openhab/
`
...
sudo chown -R True:True /var/lib/openhab/
...
sudo tar -czf backup-openhab-$(date +%Y%m%d).tar.gz /var/lib/openhab/

# Restaurer une sauvegarde
sudo tar -xzf backup-openhab-20240101.tar.gz -C /
`
...
# Mettre Ã  jour
sudo apt upgrade openhab  # Debian/Ubuntu
sudo dnf upgrade openhab  # RedHat/Fedora
...
sudo cp /etc/openhab/config /etc/openhab/config.bak

# RÃ©installer depuis les sources
sudo apt install --reinstall openhab  # Debian/Ubuntu

# Ou, restaurer depuis le paquet
sudo apt-file extract openhab /etc/
`
